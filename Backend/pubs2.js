...
async function PubscrapeUTMScholar() {
  let browser;
  let timeout;
  ...
  try {
    ...
    // Set a timer to stop the script after 5 minutes
    timeout = setTimeout(() => {
      timeoutReached = true;
        ...
        try {
          ...
          const publicationsData = await page.evaluate(() => {
            ...
            const data = rows.map((row) => {
              ...
              const journalName =
                cells[0]?.querySelector("a")?.textContent.trim() || "N/A";
...
module.exports = PubscrapeUTMScholar;
